...
func (c *upstreamChecker) CheckReadiness(ctx context.Context) error {
	...
	// Try /health/ready first, fallback to / if not available
	...
	if err != nil {
		// If /health/ready fails, try the liveness endpoint as fallback
		// Create a new context for the fallback request
		fallbackCtx, fallbackCancel := context.WithTimeout(ctx, 1*time.Second)
		defer fallbackCancel()

		fallbackURL := c.baseURL + "/"
		fallbackReq, fallbackErr := http.NewRequestWithContext(fallbackCtx, "GET", fallbackURL, nil)
		if fallbackErr != nil {
			return err // Return original error
		}
		resp, err = c.client.Do(fallbackReq)
		if err != nil {
			return err
		}
